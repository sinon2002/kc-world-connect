  ...
  return (
    <>
      <section className="relative overflow-visible pt-12 pb-6 text-white" style={{ background: "#0078c3" }}>
        ...
        {/*
          Главный контейнер.
          На телефонах — вертикальный Flexbox (flex-col) с четким разделением блоков (gap-8).
          На больших экранах (lg:) — привычный трёхколоночный Grid.
        */}
          ...
          <div className="w-full">
            <Reveal>
              ...
              <div className="mt-6 flex gap-3">
                <a href={`https://wa.me/${whatsappNumber}`} target="_blank" rel="noopener noreferrer" aria-label="WhatsApp" className="inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-white/10 transition-colors hover:bg-white/20" >
              ...
              <div className="rounded-3xl bg-white p-6 shadow-2xl sm:p-8">
                {sent ? (
                  <div role="status" className="py-10 text-center" >
                    <p className="font-display text-xl font-bold text-ink"> Спасибо, {form.name.trim()}! </p>
                    <p className="mt-3 text-muted-foreground"> Ваша заявка открыта в WhatsApp. <br /> Осталось только нажать кнопку отправки сообщения. </p>
                    <button type="button" onClick={() => { setSent(false); setForm({ name: "", phone: "", country: "", }); }} className="mt-6 text-sm font-semibold text-blue underline" > Отправить ещё одну заявку </button>
                  </div>
                ) : (
                  <form className="space-y-5" noValidate onSubmit={handleSubmit} >
                    {/* Имя */}
                    <div>
                      <label htmlFor="kc-name" className="block text-sm font-semibold text-ink" > Имя </label>
                      <input id="kc-name" name="name" required autoComplete="name" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value, })) } className="mt-2 w-full rounded-2xl border border-black/10 bg-cream px-4 py-3 text-ink outline-none transition focus:border-blue focus:ring-2 focus:ring-blue/20" placeholder="Айсулуу" />
                    </div>
                    {/* Телефон */}
                    <div>
                      <label htmlFor="kc-phone" className="block text-sm font-semibold text-ink" > Телефон / WhatsApp </label>
                      <input id="kc-phone" name="phone" type="tel" required autoComplete="tel" value={form.phone} onChange={(e) => setForm((f) => ({ ...f, phone: e.target.value, })) } className="mt-2 w-full rounded-2xl border border-black/10 bg-cream px-4 py-3 text-ink outline-none transition focus:border-blue focus:ring-2 focus:ring-blue/20" placeholder="+996 700 000 000" />
                    </div>
                    {/* Страна */}
                    <div>
                      <label htmlFor="kc-country" className="block text-sm font-semibold text-ink" > Интересующая страна </label>
                      <select id="kc-country" name="country" value={form.country} onChange={(e) => setForm((f) => ({ ...f, country: e.target.value, })) } className="mt-2 w-full rounded-2xl border border-black/10 bg-cream px-4 py-3 text-ink outline-none transition focus:border-blue focus:ring-2 focus:ring-blue/20" >
                        <option value=""> Выберите страну </option>
                        {countries.map((c) => ( <option key={c} value={c}> {c} </option> ))}
                      </select>
                    </div>
                    {/* Кнопка */}
                    <button type="submit" disabled={!valid} className="w-full rounded-2xl bg-gold px-6 py-4 font-display text-base font-bold text-navy shadow-lg transition-all hover:-translate-y-0.5 hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-50" > Отправить заявку </button>
