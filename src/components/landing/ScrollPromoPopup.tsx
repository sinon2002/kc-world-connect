...
export function ScrollPromoPopup() {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Бесплатная консультация" onClick={() => setDismissed(true)} className="fixed inset-0 z-[80] flex items-center justify-center bg-ink/60 p-4 backdrop-blur-sm">
      <div onClick={(e) => e.stopPropagation()} className="relative flex w-full max-w-sm flex-col items-center overflow-hidden rounded-3xl bg-white px-6 pb-8 pt-14 text-center shadow-2xl sm:max-w-md sm:flex-row sm:gap-6 sm:px-8 sm:text-left">
        <button type="button" onClick={() => setDismissed(true)} aria-label="Закрыть" className="absolute right-4 top-4 grid size-9 place-items-center rounded-full bg-secondary text-ink transition-colors hover:bg-secondary/70">
        ...
        <div className="mt-2 sm:mt-0">
          <p className="font-display text-xl font-extrabold text-ink sm:text-2xl">Получите бесплатную консультацию</p>
          <p className="mt-2 text-sm text-muted-foreground">Расскажем, какой вуз и страна подойдут именно вам — бесплатно и без обязательств.</p>
          <div className="mt-5 flex items-center justify-center gap-3 sm:justify-start">
            <a href={whatsappHref} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-[#25D366] px-5 py-3 text-sm font-bold text-white shadow-soft transition-transform hover:-translate-y-0.5">
            ...
            <a href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer" aria-label="Instagram Kyrgyz Concept" className="inline-flex size-11 items-center justify-center rounded-full bg-secondary text-ink transition-colors hover:bg-secondary/70">
